...
def save_samples(noisy_input_test, denoised_images1, denoised_images2,pure_test, img_height=256, img_width=256):
    ...
    axes[0,2].set_title('Res UNet denoised image')
    axes[1,0].set_title('CBAM UNet denoised image')
    axes[1,1].set_title('NL Means denoised image')
    axes[1,2].set_title('BM3D denoised image')
    pure_images = []
    noisy_images = []
    bm3d_images = []
    nl_images = []
    ...
    for i in range(0, len(noisy_input_test)):
        ...
        BM3D_start_time = time.time()
        bm3d_denoised = bm3d_denoise_rician_dipy(noisy_image)
        BM3D_inference_time = time.time() - BM3D_start_time
        print('BM3D process time:', BM3D_inference_time)
        bm3d_time.append(BM3D_inference_time)
        bm3d_denoised = np.clip(bm3d_denoised, 0, 1)
        NLmeans_start_time = time.time()
        nl_denoised = nlm_denoise_dipy(noisy_image)
        NLmeans_inference_time = time.time() - NLmeans_start_time
        nl_time.append(NLmeans_inference_time)
        print('NLmeans process time:', NLmeans_inference_time)
        nl_denoised = np.clip(nl_denoised,0, 1)
        ...
        axes[0,2].imshow(denoised_image1, pyplot.cm.gray)
        axes[0,2].set_xlabel("SSIM: {:.5f} PSNR: {:.5f}".format(get_image_ssim(pure_image, denoised_image1,data_range=1.0),PSNR(pure_image, denoised_image1,max_val=1.0)))
        axes[1,0].imshow(denoised_image2, pyplot.cm.gray)
        axes[1,0].set_xlabel("SSIM: {:.5f} PSNR: {:.5f}".format(get_image_ssim(pure_image, denoised_image2,data_range=1.0),PSNR(pure_image, denoised_image2,max_val=1.0)))
        axes[1,1].imshow(nl_denoised, pyplot.cm.gray)
        axes[1,1].set_xlabel("SSIM: {:.5f} PSNR: {:.5f}".format(get_image_ssim(pure_image, nl_denoised,data_range=1.0),PSNR(pure_image, nl_denoised,max_val=1.0)))
        axes[1,2].imshow(bm3d_denoised, pyplot.cm.gray)
        axes[1,2].set_xlabel("SSIM: {:.5f} PSNR: {:.5f}".format(get_image_ssim(pure_image , bm3d_denoised,data_range=1.0),PSNR(pure_image , bm3d_denoised,max_val=1.0)))
        fig.suptitle("Rician noise σ=0.05",fontsize=20, fontweight='bold')
        plt.savefig("results/img{0}.png".format(i))

    plt.close()
    

    print('Each slice process time for BM3D:',np.mean(bm3d_time),'second')
    print('Each slice process time for NL-means:',np.mean(nl_time),'second')
    ...
    n3 = get_set_ssim(np.array(pure_images) , np.array(bm3d_images), img_height, img_width)
    n4 = get_set_ssim(np.array(pure_images) , np.array(nl_images), img_height, img_width)
...
_ = model_Unet.predict(samples)

startCBAM= time.time()
denoised_images_CBAMunet = model_CBAMunet.predict(samples)
inferCBAM=time.time()-startCBAM
print('CBAMUNet infer time:',inferCBAM)

startres= time.time()
denoised_images_res = model_res.predict(samples)
inferres=time.time()-startres
print('ResUnet infer time:',inferres)




print('plotting')
save_samples(samples, denoised_images_res,denoised_images_CBAMunet, pure_test,img_width=img_width, img_height=img_height)
...
def BM3D_on_test_set(test_files, img_height=256, img_width=256, batch_size=10):
    ...
    for i in range(len(X_test)):
        ...
        single_image = X_test[i,:,:,0]
        denoised.append(bm3d_denoise_rician_dipy(single_image))
    

    inference_time = time.time() - start_time
    
    denoised = np.array(denoised)[:,:, np.newaxis]
    ...
    print("BM3D Model performance on test set, SSIM: {0}, PSNR:{1}".format(avg_ssim,avg_psnr))
    print(f'Inference time: {inference_time:.2f}s total')
    
    return avg_ssim,avg_psnr

def NLmeans_on_test_set(test_files, img_height=256, img_width=256, batch_size=10):
    ...
    for i in range(len(X_test)):
        ...
        single_image = X_test[i,:,:,0]
        denoised.append(nlm_denoise_dipy(single_image))
    ...
    print("NLmeans Model performance on test set, SSIM: {0}, PSNR:{1}".format(avg_ssim,avg_psnr))
    print(f'Inference time: {inference_time:.2f}s total')
...
#for model, model_name in [(model_res, "ResUNet"), (model_CBAMunet, "CBAM UNet")]:
