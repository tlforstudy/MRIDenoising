...
for model, model_name in [(model_CBAMunet, "CBAM UNet")]:
    for sigma in [0.01,0.03,0.05,0.07,0.09]:
        print(f"\nEvaluating model: {model_name}, sigma= {sigma}")
        evaluate_on_test_set(model, test_files,rician_noise_sigma=sigma, model_name=model_name)
'''
'''        
for model, model_name in [(model_CBAMunet, "CBAM UNet")]:
    for sigma,kernelsize in [(0.2,3),(0.4,3),(0.6,5),(0.8,5),(1.0,7)]:
...
'''
for model, model_name in [(model_CBAMunet, "CBAM UNet")]:
    for sigma,kernelsize in [(1.2,9),(1.5,11)]:
        print(f"\nEvaluating model: {model_name}, sigma= {sigma},kernel_size = {kernelsize}")
        evaluate_on_test_set(model, test_files,blur_sigma=sigma,kernel_size=kernelsize, model_name=model_name)'''
