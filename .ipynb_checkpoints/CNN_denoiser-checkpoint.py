...
class CNN_denoiser():
    def __init__(self, batch_size=10, nu_epochs=50, validation_split=0, img_height=256, img_width=256,model_type=0):
        img_width, img_height = img_height, img_width
        self.batch_size = batch_size
        self.nu_epochs = nu_epochs
        self.validation_split = validation_split
        if model_type == 1:
            self.model = autoencoder.get_autoencoder_model_upsample(img_width=img_width, img_height=img_height)
        elif model_type == 2:
            self.model = autoencoder.get_autoencoder_model_transpose(img_width=img_width, img_height=img_height)
        elif model_type == 3:
            self.model = autoencoder.get_autoencoder_model_NOCBAM(img_width=img_width, img_height=img_height)
        elif model_type == 4:
            self.model = autoencoder.get_autoencoder_model_noskipnoCBAM(img_width=img_width, img_height=img_height)
        elif model_type == 5:
            self.model = autoencoder.get_autoencoder_model_CBAMencoder(img_width=img_width, img_height=img_height)
        else:
            print('Error')
    ...
    def model_plots(self, noise_prop, noise_mean, noise_std):
        ...
        plt.show()
        plt.savefig("results/Model loss.png")

    @staticmethod  # Split 2 datasets
    def train_test_split(set1, set2, train_split=0.9, shuffle_test_set=False, img_height=256, img_width=256):
    ...
    def train(self, noisy_input, pure,batchsize, save=False, verbosity=0):
    ...
    def train_with_generator(self, train_generator, val_generator=None, steps_per_epoch=100,validation_steps=10, save_path='./models'):
