            ...
            for file in batchfile:
                ...
                try:
                    ...
                    for slice_idx in selected_slices:
                        ...
                        noisy_slice = add_gaussian_noise(clean_slice.copy(), noise_mean=noise_mean, noise_std=noise_std, noise_prop=noise_prop)
...
def save_samples(noise_vals, noisy_input_test, denoised_images1, denoised_images2,pure_test, img_height=256, img_width=256):
    ...
    for i in range(0, len(noisy_input_test)):
        ...
        axes[4].set_xlabel("SSIM: {:.5f} PSNR: {:.5f}".format(get_image_ssim(pure_image, nl_denoised,data_range=1.0),PSNR(pure_image, nl_denoised,max_val=1.0)))
        axes[5].imshow(bm3d_denoised, pyplot.cm.gray)
        axes[5].set_xlabel("SSIM: {:.5f} PSNR: {:.5f}".format(get_image_ssim(pure_image , bm3d_denoised,data_range=1.0),PSNR(pure_image , bm3d_denoised,max_val=1.0)))
...
print('plotting')
#save_samples((noise_prop, noise_std, noise_mean), samples, denoised_images_CBAMunet,denoised_images_CBAMunet, pure_test,img_width=img_width, img_height=img_height)
        ...
        try:
            with np.load(npz_file) as data:
                ...
                for slice_idx in valid_slices:
                    ...
                    clean_slice = resize(clean_slice, (256, 256), anti_aliasing=True)
                    #noisy_slice = clean_slice.copy()
...
def evaluate_on_test_set_gaussian(model,test_files,model_name=None,img_height=256, img_width=256, batch_size=10):
...
def read_ixi_t2_z(file_list,batch_size=10,max_attempts=5,slice_thresh=0.1,low_percent=5,high_percent=95):
    num = len(file_list)
    while True:
        for i in range(0, num, batch_size):   
            batchfile = file_list[i:i+batch_size]
...
file_list_t2 = glob.glob(os.path.join("./data/ixi-T2/", "*.nii.gz"))


#test_generator = read_ixi_t2_z(file_list=file_list_t2, batch_size=10)
...
def evaluate_on_test_set_t2(model,test_files,model_name=None,img_height=256, img_width=256, batch_size=10):
    ...
    for nii_file in test_files:
        try:
            ...
            for slice_idx in range(num_slices):
                clean_slice = data_norm[:, :, slice_idx]
            ...
            print(f"Error loading {nii_file}: {str(e)}")
            continue
...
#for model, model_name in [(model_CBAMunet, "CBAM UNet")]:
    #print(f"\nEvaluating model: {model_name}")
    #evaluate_on_test_set_t2(model, file_list_t2, model_name=model_name)